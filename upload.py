...
def handle_dialog(res, req):
    ...
    if 'помощь' in req['request']['nlu']['tokens']:
        res['response'][
            ...
            В момент игры у тебя будет две попытки угадать город, но не бойся тебе хватит и одной попытки ведь игра совсем не сложная, а теперь ответь пожалуйста на предыдущий вопрос'''
        return

    if 'покажи город на карте' == req['request']['command']:
        ...
        if not sessionStorage[user_id]['game_started']:
            if 'да' in req['request']['nlu']['tokens']:
                ...
                res['response']['text'] = sessionStorage[user_id][
                                              'first_name'] + ', я не поняла твоего ответа! Так да или нет?'
                res['response']['buttons'] = [
                    {
                        'title': 'Да',
                        'hide': True
